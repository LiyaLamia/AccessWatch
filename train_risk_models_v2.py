...
# Identifier columns expected across the model tables
ID_COLS = ["raion_id", "raion_name", "oblast_name", "week_start"]

# Mapping used for the naive persistence baseline:
# predict next week using the current week's same signal.
TARGET_MAP_NAIVE = {
...
def normalize_id_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    Normalize key identifier columns so merges are less fragile.
    In particular, parse week_start as datetime and cast ID/name fields to strings.
    """
    out = df.copy()
...
def load_feature_spec(path: str) -> Dict[str, Any]:
    """
    Load the feature specification JSON. Some files wrap the spec
...
def detect_task_type(y: pd.Series, arg_task_type: str) -> str:
    """
    Auto-detect classification vs regression from the target values
    ...
    # If the target only contains 0/1 values, treat it as classification
...
def assign_time_splits(df: pd.DataFrame, valid_weeks: int, test_weeks: int) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """
    Create train/valid/test splits using trailing blocks of weeks.
    The newest weeks become test, the block before that becomes validation,
    and all earlier weeks become training.
    """
    out = df.copy()
...
def get_feature_sets(feature_spec: Dict[str, Any], model1_df: pd.DataFrame, model2_df: pd.DataFrame) -> Tuple[List[str], List[str]]:
    ...
    If the spec is missing them, fall back to inferring them from the tables.
    """
...
def to_numeric_frame(df: pd.DataFrame, feature_cols: Iterable[str]) -> pd.DataFrame:
    """
    Convert all feature columns to numeric values, coercing anything invalid to NaN.
    """
...
def split_xy(df: pd.DataFrame, feature_cols: List[str], target_col: str) -> Dict[str, Any]:
    """
    Build X, y, metadata, and boolean masks for train/valid/test.
    Rows with missing targets are dropped here.
    """
...
def build_linear_model(task_type: str, seed: int) -> Pipeline:
    ...
    logistic regression for classification, ridge regression for regression tasks.
    """
...
def build_tree_model(task_type: str, seed: int, force_fallback_tree: bool) -> Any:
    """
    Build the tree-based model.
    Prefer LightGBM when available unless fallback sklearn boosting is forced.
    """
...
def get_scores(model: Any, X: pd.DataFrame, task_type: str) -> np.ndarray:
    """
    Return scores in a consistent format:
    probabilities for classification and raw predictions for regression.
    """
    if task_type == "classification":
...
def choose_best_threshold(y_true: np.ndarray, y_score: np.ndarray) -> float:
    """
    Pick the validation threshold that gives the best F1 score.
    """
...
def compute_classification_metrics(y_true: np.ndarray, y_score: np.ndarray, threshold: float, top_k: int, top_frac: float, meta: Optional[pd.DataFrame] = None) -> Dict[str, Any]:
    """
...
def compute_ranking_metrics(df: pd.DataFrame, top_k: int, top_frac: float) -> Dict[str, Any]:
    """
    Compute top-k and top-fraction ranking metrics within each week.
    This is useful when the goal is to rank the most at-risk raions each week.
    """
    if df.empty:
        return {}
...
def get_feature_importance(model: Any, feature_cols: List[str], X_valid: pd.DataFrame, y_valid: pd.Series, task_type: str) -> pd.DataFrame:
    """
    Extract feature importance in a few different ways depending on the model:
    coefficients for linear models, native importance for tree models,
    ...
    # Final fallback: permutation importance on a validation subset
    if len(X_valid) > 0:
...
def serialize_value(v: Any) -> Any:
    """
    Convert NumPy/pandas scalar types into regular Python values
    so they can be written safely to JSON.
    """
...
def save_json(obj: Dict[str, Any], path: Path) -> None:
    """
    Save a dictionary as JSON after normalizing non-standard scalar types.
    """
...
def save_predictions(meta: pd.DataFrame, y_true: pd.Series, y_score: np.ndarray, task_type: str, threshold: Optional[float], path: Path) -> None:
    """
...
def train_one_experiment(
    ...
    """
    Train and evaluate one experiment setup,
...
def run_naive_baseline(master_df: pd.DataFrame, target_col: str, task_type: str, top_k: int, top_frac: float, outdir: Path) -> Optional[Dict[str, Any]]:
    """
    Run a simple persistence baseline using the mapped current-week source signal
    as the prediction for next week.
    """
...
def main() -> None:
    ...
    # Normalize ID/date columns before doing split assignment and merges
    master = normalize_id_columns(pd.read_csv(args.master_csv))
    model1 = normalize_id_columns(pd.read_csv(args.model1_csv))
    model2 = normalize_id_columns(pd.read_csv(args.model2_csv))
    ...
    merge_keys_model1 = [c for c in ID_COLS if c in model1.columns and c in master.columns]
    merge_keys_model2 = [c for c in ID_COLS if c in model2.columns and c in master.columns]

    master_split_cols_1 = merge_keys_model1 + ["split"]
    master_split_cols_2 = merge_keys_model2 + ["split"]

    # Drop any pre-existing split column before re-merging the canonical split labels
    if "split" in model1.columns:
        model1 = model1.drop(columns=["split"])
    if "split" in model2.columns:
        model2 = model2.drop(columns=["split"])

    model1 = model1.merge(master[master_split_cols_1], on=merge_keys_model1, how="left", validate="many_to_one")
    model2 = model2.merge(master[master_split_cols_2], on=merge_keys_model2, how="left", validate="many_to_one")
    ...
    # Build one flat comparison table across all experiments and splits
    comparison_rows = []
